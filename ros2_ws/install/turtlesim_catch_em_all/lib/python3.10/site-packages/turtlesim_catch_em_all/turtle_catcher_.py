...
class TurtleCatcherNode(Node):
    ...
    def populate_target(self,msg):
        msg = TargetList()
        target = [msg.new_target,msg.name]
        self.target_list_.append(target)
        
    def choose_target(self):
        self.target_list_.sort(key=self.euclidean_dist)
        self.turtle_control_call(self.target_list_[0])
    
    def turtle_control_call(self,target):
        ...
        future.add_done_callback(partial(self.kill_call,name=target[2]))
